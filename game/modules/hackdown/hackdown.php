...
if ($hackdownTask["id"]) {
    ...
    include("includes/class/qclass.php");
  
 
			
    $qclass = new QClass($hackdownTask);
    
    $templateVariables["qtask"] = $hackdownTask;
   		
   	$templateVariables["display"] = 'quests/quest_play.tpl';

	  
	 $templateVariables['hackdown'] = $hackdown;
...
else
{
	...
	if ($GET['rankings'])
	{
		...
		if ($GET['organizations'])
		{
			...
			$templateVariables['organizations'] = $organizations;
		}
		elseif ($GET['zones'])
		{
		...
		$templateVariables['lastDate'] = date("d/F", $hackDownId );
		$templateVariables['display'] = 'hackdown/hackdown_rankings.tpl';
	}
	else
	{
		...
		if (!$hackdownRemaining)
		{
			...
			$templateVariables['nextSaturdayRemaining'] = $nextSaturdayRemaining;
		...
		else
		{
			...
			if ($arena['arena_id'])
			{
				$templateVariables['inArena'] = $arena;
			}
			else
			{
				if ($_POST['arena'])
				...
				$hacks=$hacks[0];
				$templateVariables['hacks'] = $hacks;
			
			}
				$templateVariables['hackdownRemaining'] = $hackdownRemaining;
		}
		$templateVariables['display'] = "hackdown/hackdown.tpl";
	}
