...
if ($GET['history'])
{
	...
	if ( !$_SESSION['premium']['consoleHistory'] )
		$errors[] = "You need to <a href='".URL."alpha_coins/option/consoleHistory'>activate the Console History</a> before using it. You will gain access to your full history and in case of party missions you will be able to view the history of all participants.";
	else
	{
		...
		if ($qtask['instance_id']) 
			$db->where('instance_id', $qtask['instance_id']);
		...
		$templateVariables['currentParticipant'] = $currentParticipant;
		$templateVariables['history'] = $history;
	}
	if ($qtask['instance_id']) 
	{
		$participants = $db->where('in_party', $user['in_party'])->get('users', null, 'username, id');

		$templateVariables['participants'] = $participants;
	}
	$templateVariables["display"] = 'quests/quest_console_history.tpl';
}
else
{
    ...
    if ($qtask["type"] == 15 && $qclass->finished) {
		...
		$u = $goliath_mmk->post_request('user/create/user_' . $user['id'] . '/user_' . $user['id'] . '@secretrepublic.net');

		$t = $goliath_mmk->put_request('activity/user/'.$u['id'].'/game/1/stat/'.($qclass->questTask["questID"] + 67).'/data/1');
		
		
      
	  $success[] = "Mission completed";
		
      $quest = $db->join('quests_user qu', 'qu.quest = q.id and qu.user_id = ' . $user['id'], 'left outer')->where("q.id", $qclass->questTask["questID"])->getOne("quests q", "qu.id qu_id, type, money, experience, achievement_id, skillPoints, qu.created as done, times");
      
      $giveRewards = true;
      
      if ($quest['type'] == 2 && $quest['done'])
        $giveRewards = false;
      ...
      if ($giveRewards) {
        ...
        if (isset($qclass->questData["rewards"]["achievements"]))
          $achievements = $qclass->questData["rewards"]["achievements"];
        
        if ($quest["achievement_id"])
          $achievements[] = $quest["achievement_id"];
        ...
        if ($user['in_party']) {
          ...
          foreach ($participants as $participant)
            $uclass->addReward($participant["id"], $reward, $qclass->questTask["name"]);
        } //$user['in_party']
        else
          $uclass->addReward($user["id"], $reward, $qclass->questTask["name"]);
        
        
      } //$giveRewards
      
      if ($qclass->questTask['instance_id']) {
        $db->where('instance_id', $qclass->questTask['instance_id'])->delete('party_quest_instances');
    ...
    $templateVariables["qtask"]         = $qtask;
    $templateVariables['showPartyChat'] = true;
    $templateVariables["display"]       = 'quests/quest_play.tpl';
