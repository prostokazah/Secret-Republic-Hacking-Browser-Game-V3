...
if ($hackTask["id"]) {
    ...
    $templateVariables["qtask"] = $hackTask;
...
else
{
  ...
  if (!$lastHack['created'] && $_POST['hack'])
    if ($uclass->canDoTask())
      ...
      $templateVariables['nrmUpgradeCost'] = $nrmUpgradeCost;
      }
    if ($GET['history'])
    {
    ...
    elseif ($GET['rankings'])
    {
      ...
      $templateVariables['lastHack'] = $lastHack;
      $templateVariables['load'] = 'hackingPoints';
}