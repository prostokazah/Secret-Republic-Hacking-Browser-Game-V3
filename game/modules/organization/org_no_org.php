<?php
  ...
  else
  {
    ...
    $templateVariables['organizationMission'] = sprintf('[%s - %s (level %s)]%s', $mission['name'], $mission['title'], $mission['level'], $mission['done']?' (done)':''); 
    $templateVariables['canCreateOrganization'] = $canCreateOrganization; 
    $templateVariables['organizationCost'] = $oclass->organizationCost; 
    $templateVariables['availableOrgs'] = $availableOrgs; 
  }