...
$templateVariables['maxServers'] = $maxServers;
...
if (isset($GET['build']))
{
	...
	if (!count($errors))
	{
		...
		$templateVariables['storage'] = $storage;
	}
	
	$templateVariables['serversCount'] = $serversCount['nrs'];
	$templateVariables['cost'] = $cost;
	$templateVariables['canBuild'] = $canBuild;
	$templateVariables['display'] = 'servers/build_server.tpl';
}
elseif (isset($GET['server']))
{
	...
	if ($GET['change'] == "hostname")
	{
		...
		$templateVariables['server'] = $server;
		$templateVariables['display'] = "servers/server_change_hostname.tpl";
	}
	elseif ($GET['transfer'])
	{
		...
		$templateVariables['display'] = 'servers/server_transfer_app.tpl';
	}
	else
	{
...
//$server->recomputeServerResources();
		...
		$templateVariables['server'] = $server;
		$templateVariables['display'] = 'servers/server.tpl';
	}
}
else
{
	...
	$templateVariables['servers'] = $servers;
	$templateVariables['display'] = 'servers/servers.tpl';
}
