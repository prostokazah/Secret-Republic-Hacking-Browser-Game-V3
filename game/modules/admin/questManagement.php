...
if ($quest["id"]) {
  ...
  switch ($GET["load"]) {
    case 'stats':
	  ...
	  $templateVariables['feedback'] = $feedback;
      $templateVariables['stats'] = $stats;
    break;
    case "objectives":
      $templateVariables["achievements"] = $db->get("achievements");
    ...
    default:
      ...
      $templateVariables["quests"] = $db->where('id', $quest['id'], '!=')->get("quests", null, 'title, id');
      $templateVariables["groups"] = $db->get("quest_groups", null, 'name, qgroup_id');
  } //$GET["load"]
  
  //$aclass->files($quest);
  
  
  $templateVariables["quest"] = $quest;
  
  
  $templateVariables["display"] = 'admin/quests/quest_manage.tpl';
...
elseif ($GET['tree'])
{
	...
	$templateVariables['tree'] = $tree;
	$templateVariables['display'] = 'admin/quests/questsTree.tpl';
}
elseif ($group_id = $GET['group'])
  {
	  ...
	  $templateVariables['quests'] = $db->where('qgroup_id != ?',array($group_id))->get('quests', null, 'title, id');
	  $templateVariables['group'] = $group;
	  $templateVariables['display'] = "admin/quests/questGroupCreate.tpl";
}
else
{
...
$groups = $db->rawQuery(sprintf(
  ...
  $templateVariables["display"] = 'admin/quests/questList.tpl';
}
