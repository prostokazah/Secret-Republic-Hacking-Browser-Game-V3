...
if ($hacker['id']) {
  ...
  $templateVariables['credentials'] = $hackerCredentials;
  
  if ($hackerCredentials['uid']) {
	if ($GET['load'] == 'attacks') {
      $templateVariables['load'] = 'attacks';
	}
    ...
    elseif ($GET['load'] == 'credentials' && $user['cardinal'])
    {
      ...
      $templateVariables['load'] = 'credentials';
    ...
    }elseif ($GET['load'] == 'bans' )
    {
      ...
      for ($i=1;$i<=12;$i++)
        $templateVariables["months"][] = date("F", mktime(0, 0, 0, $i+1, 0, 0, 0));
      ...
      $templateVariables['bans'] = $bans;
      $templateVariables['load'] = 'bans';
      
    }elseif ($GET['load'] == 'missions') {
      $templateVariables['load'] = 'missions';
      ...
      $templateVariables['missions'] = $missions;
    } //$GET['load'] == 'missions'
      elseif ($GET['load'] == 'achievements') {
      $templateVariables['load'] = 'achievements';
      ...
      $templateVariables['userAchievements'] = $userAchievements;
      $templateVariables['achievements']     = $achievements;
      
    } elseif ($GET['load'] == 'skills') {
    
      $templateVariables["load"] = "skills";
      if ($_POST)
      {
      ...
      $templateVariables["skills"] = $db->where("uid", $hacker["id"])->get("skills");
    ...
    $templateVariables['display'] = 'admin/hacker/hacker.tpl';
