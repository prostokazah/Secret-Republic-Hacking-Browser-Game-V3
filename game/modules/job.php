...
if (!defined('cardinalSystem')) exit;	
  ...
  include("includes/constants/jobs.php");
  ...
  $qclass = new QClass($jobTask);
   ...
   if($jobTask["id"]){
    ...
    $templateVariables["qtask"] = $jobTask;
  ...
  if (!$jobTask["id"])
  {
    ...
    $templateVariables["timeBetweenJobs"] = $config['timeBetweenJobs'];
    $templateVariables["job"] = $job;
    $templateVariables["jobs"] = $jobs;
  
    $templateVariables["display"] = "job/job.tpl";
  }
