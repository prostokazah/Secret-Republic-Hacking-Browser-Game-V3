...
if ($GET['history'])
{
	...
	$counts = $db->rawQuery('select (select count(*) from alpha_coin_transactions where user_id = ?) nrACT, (select count(*) from alpha_coins_logs where user_id = ?) nrACS',
	...
	$shop_logs      = $db->join("alpha_coins_shop acs", "acs.item_id = acsl.item_id", "left outer")
		                 ...
		                 ->orderBy("acsl.created", 'desc')
		                 ->get("alpha_coins_logs acsl", $pages->limit, "acs.name, acsl.*");
	$transactions_logs = $db->where('user_id', $user_id)
		                    ->orderBy("act.created", 'desc')
	...
	$templateVariables['shop_logs'] = $shop_logs;
	$templateVariables['transactions_logs'] = $transactions_logs;
	$templateVariables['display'] = 'alpha_coins/alpha_coins_history.tpl';
}
else
{
	...
	{
		global $db, $uclass, $goliath;

		$item = $db->where('available', 1)->where('item_id', $item_id)->getOne("alpha_coins_shop");
	  ...
	  elseif ($item['type'] == 2)
	  {
	  	$items = $db->where('available', 1)->where('type', 1)->get('alpha_coins_shop');
	  	foreach($items as $anItem)
	  		addTimeToColumn($dataUserPremiumUpdate, $premium, $anItem, $item['extraData']);
	  ...
	  $goliath->setUser($user_id);
	
	  $log = $goliath->log()
              ->add('purchased', 'premium_item', $item_id)
			  ->add('paid', 'usd', $item['monthly_cost']);
...
if (!$_SESSION['alphaSpecialOffers'])
	{
		...
		if (!$check['transaction_id']) 
			$myModal['content'] .= "<div class='well'>" . $firstBuyNotice . "</div>";
	...
	$items = $db->where('available', 1)->orderBy('shopOrder', 'asc')->get("alpha_coins_shop");


	foreach ($items as &$item)
	{
	  if ($premium[$item['user_premium_col']] > time())
		$item['alreadyActive'] = date('d/F/Y H:i', $premium[$item['user_premium_col']]);
	} 
...
$templateVariables['display'] = 'alpha_coins/alpha_coins_options.tpl';
}