...
if (GET('details'))
{
  ...
  $templateVariables['display'] = 'rankings/details.tpl';
}	
else
{
  ...
  if(!$smarty->isCached('rankings/rankingsGrid.tpl', $cache_id) )
  {
    ...
    if($GET["type"]=="orgs"){
      ...
      $rankings["topUsers"] = $db->where("rank", array(">" => 0))
                                 ->orderBy("rank", "asc")->get("users", 3, "username,gavatar, id ".$rankSelect);
    
      if ($zone) $db->where("zone", $zone);
    
      $users = $db->where("rank", array(">" => 0))->getOne("users", "count(*) nru");
      $pages = new Paginator;
      ...
      $users = $db->where("rank", array(">" => 0))->get("users", $pages->limit, "id, username, level, zone, points, gavatar".$rankSelect);
  ...
  $templateVariables["rankingsGrid"] = $smarty->fetch("rankings/rankingsGrid.tpl", $cache_id);
  $smarty->caching = 0;
	$templateVariables["display"] = 'rankings/rankings.tpl';
}
