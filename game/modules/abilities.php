...
require_once('includes/class/abclass.php');
  ...
  /* /ABILITIES TASK  */
	...
	if($abtask){
	  ...
	  if ($abtask["remainingSeconds"]>60*60)
	  {
		...
		$templateVariables["abtask"] = $abtask;
  ...
  else
  {
    ...
    if(ctype_digit($_POST["a"]) && !$abtask["id"] && !$a['cant'])
	  ...
	  $templateVariables["userAbilities"] = $abclass->userAbilities;
	  $templateVariables["abilities"] = $abclass->abilities;
	}

  $templateVariables["display"] = "skills_and_abilities/abilities.tpl";
