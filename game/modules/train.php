...
require_once('includes/class/tclass.php');
  ...
  $page_title="Training facility";
	...
	if ($trainTask['dataid'] == 1)
    {
	  ...
	  $templateVariables["captcha"] = $cardinal->generate_captcha_box();
    }
...
if(!$tclass->trainTask){
  ...
  {          
	...
	else
	{
		...
		$templateVariables["trainLog"] = $trainLog;
		$templateVariables["trainEvery"] = $config['trainEvery'];
	}
  ...
  $templateVariables["trainTask"] = $tclass->trainTask;
  
  

	$templateVariables["display"] = 'train/train.tpl';
