...
class Alpha
{
  ...
  {
    global $db, $templateVariables, $info,$pages, $voice, $config, $user, $GET, $logged, $taskclass, $url, $success, $messenger, $errors, $warnings,$uclass;
    $this->db = &$db;
  ...
  {
    if (isset($type))
      $this->messenger[] = array('message' => $message, 'type' => $type);
    else
      $this->messenger[] = array('message' => $message);
  }
  function show_404(){

  $this->voice = '404';
	$this->db->reset();
	$dataInsert = array('created'=>time(), 'url' => $this->url, 'user_id' => $this->user['id']);
  ...
  function redirect($url, $keepPostData = false) {
    ...
    META;*/
	  ...
	  if ($this->voice) $_SESSION['voice'] = $this->voice;

	  if ($myModals[0]) $_SESSION['myModal'] = $myModals[0];
