...
class BattleSystem extends Alpha
{
	...
	function computePlayerStats($user_id, $computeLayers = true, $computeSpy = false, 
		$skills = false, $servers = false, $serversUsed = array(),
		$dontTakeUsedServersInAccount = false)
	{
		...
		if (!$servers)
		{
			...
			if ($dontTakeUsedServersInAccount)
			{
				...
				foreach($servers as $key => $s)
					if ($s['cant_use']) unset($servers[$key]);
			}
		...
		foreach ($player['skills'] as $skillID => $level)
			if (isset($theskills[$skillID]["spy"]))
				...
				function executeLayersFight($attacker, $defender)
				{
					...
					while (    $attacker['defense'] > 0 and $attacker['defense'] > 0 
						and $defender['attack']  > 0 and $defender['attack']  > 0)
					{
						$round = array();
					...
					return array("breaching" => $breachingPercentage, "rounds" => $rounds);
				}
		  ...
		  if ($report['brokenLayers'] >= 2)
		  {
		  	...
		  	if ( $computeSendPenaltiesAndRewards )
		  	{
		  		...
		  		$this->uclass->updatePlayer(array('money' => $recData['money'] - $lostMoney,
		  		...
		  		$this->db->rawQuery('update zone_grid_cluster_nodes set floatingDataPoints = floatingDataPoints + ?
		  			where zone_grid_cluster_nodes_id = ? limit 1',
		  			array($floatingDataPoints, $receiver['zone_grid_cluster_nodes_id']));
		  ...
		  $report['serversDamage'] = array(
		  	'sender' => array('damage' => $senderDamage, 'softwareDamage' => $senderSoftwareDamage),
		  	'receiver' => array('damage' => $receiverDamage, 'softwareDamage' => $receiverSoftwareDamage)
		  	);
