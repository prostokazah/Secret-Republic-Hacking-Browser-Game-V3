...
class Admin extends Alpha
{
  ...
  private $objectiveTypes = 
    array('ping', 'crack', 'connect', 'transfer', 'decrypt_file', 
          'delete_logs', 'run_file', 'delete_file', 'kill_file', 
          'nmap', 'cat', 'delete_email', 'read_email', 
          'forward_email', 'crash_service', 'sql_delete', 'sql_drop');
          
  private $serviceTypes =
    array( 'ssh', 'sql', 'smtp', 'http', 'unknown');
  ...
  function get_registered()
  {
    ...
    if ($this->GET['online']) {
      ...
      $nrMembers = $this->db->rawQuery('select count(hu.id) as nrMembers from users as hu
		                                where lastActive > ?', array(time() - 10 * 60));
    ...
    elseif (ctype_digit($this->GET['group'])) 
	{
      ...
      $nrMembers = $this->db->rawQuery('select count(hu.id) as nrMembers from users as hu 
		                                ...
		                                where uc.group_id = ?', array($this->GET['group']));
  ...
  function host_services(&$quest)
  {
    ...
    if ($service = $_POST['service']) {
      ...
      if ($_POST['deleteservice']) {
        $this->db->where('service_id', $service)->delete('quest_services');
		 $this->db->where('requiredObjective', $service)->update('quest_files', array('requiredObjective' => NULL));
      } //$_POST['deleteService']
	...
	$this->templateVariables['objectives'] = $this->db->rawQuery('select type, id, side, data,oorder from quest_objectives where qid = ? order by oorder asc', array($quest['id']));
  }
