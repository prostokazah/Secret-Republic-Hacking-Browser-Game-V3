...
class Game{
		...
		function generateNumbersPattern($hardness = 1)
		{
			...
			$pattern = array("numbers" => array(), "nth" => rand(1,6));
			//$rand=7;
			...
			switch ($rand)
			{
				...
				case 4:
					...
					while(count($extras) < $pattern['nth'])
					{
						...
						} else { $extras[] = $number; $extras[] = $number2;}
		...
		{
			if ($n < 5) return (($n < 2)? false : (($n < 3)? array(2) : array(2, 3))); 
			$primes = range(3, $n, 2);
			for ($i = 0, $sqrtN = sqrt($n), $pTot = count($primes); $j = 0, $k = $primes[$i], $k <= $sqrtN; $i++)
			{
				if ($k == null) continue;
				while (++$j * $k < $pTot) $primes[$j * $k + $i] = null;
			}
