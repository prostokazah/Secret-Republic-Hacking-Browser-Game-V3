...
class Blog extends Alpha{
    ...
    function validateBlogTitle(&$title){
      global $errors;
        ...
        }else $errors[] = "Title must be between 5 and ".$this->config["blog_title_size"]." characters";
    ...
    function fetchArticle($id){
      ...
      $article= $this->db->join('blogs b', 'b.blog_id = ba.blog_id', 'left outer')
                           ->join('users u', 'u.id=b.user_id ', 'left outer')
                           ->where('ba.article_id = ?', array($id))
    ...
    function voteUp(&$article){
      $this->db->insert("blog_article_votes", array("article_id" => $article['article_id'], "user_id" => $this->user['id'], 'created' => time()));
    ...
    function verify_create_article($article = null){
      ...
      if($_POST)
      {
        ...
        $lastArticle = $this->db->where('blog_id', $this->blog['blog_id'])
                                ->where('created', time() - 60, '>=')
                                ->getOne('blog_articles', 'article_id');
        if ($latestArticle['article_id'])
          $errors[] = 'You can only publish one article every 60 seconds.';
        else
        if($this->validateBlogTitle($title = $_POST["title"])
           && $this->validateArticleContent($content = $_POST["content"])){
    ...
    function validateArticleContent($content)
    {
      ...
      } else $errors[] = "Content must have between 100 and ".$this->config["blog_article_size"]." chars";
    }
    ...
    function check_add_comment(&$article){
      ...
      if($_POST["comment"]){
            ...
            if(strlen($content)<=500 && strlen($content)>=3){
              ...
              $this->db->rawQuery('update blog_articles set nrc=nrc+1 where article_id = ?', array($article['article_id']));

              $_SESSION["success"]="Comment added";
              $this->redirect($url);

            }  else $errors[] = "Content must have between 3 and 500 characters";
          }
    }
    ...
    function fetch_article_comments($article){
      ...
      if($article["nrc"]){
          ...
          $comments =
            $this->db->join('users u', 'u.id=bc.user_id' ,'left outer')
                     ->where('article_id', $article['article_id'])
                     ->get('blog_comments bc', $this->pages->limit, 'bc.comment_id,content,created,u.username as user');
    ...
    function manageSubscribtion($blog){
      ...
      if(submitted_form("subscribe"))
      {
        ...
        $this->db->where('blog_id', $blog['blog_id'])->update('blogs', array('nrs' => $blog['nrs']));
        $this->redirect(URL_C);
      }
    ...
    function fetch_subscribers($blog){
      global $subscribers;
          ...
          $this->pages->paginate();
          $subscribers =
            $this->db->where("bs.blog_id", $blog["blog_id"])
                     ->join("users u", "u.id = bs.user_id", "LEFT OUTER")
                     ->orderBy("created", "desc")
                     ->get("blog_subscriptions bs", $this->limit, "created,u.username  user");

          $templateVariables["subscribers"] = $subscribers;
    }
    ...
    function verify_change_name(){
      ...
      if($_POST["edit"]){
        if($this->validateBlogTitle($title = $_POST["title"])){
          $updateData = array("name" => htmlentities($title));
