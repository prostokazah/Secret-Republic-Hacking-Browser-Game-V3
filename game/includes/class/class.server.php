...
class Server extends Alpha
{
	...
	{
		 $component = $this->db->join('components c', 'c.component_id = sc.component_id' , 'left outer')
			                   ->where("server_id", $server->server_id)
				               ->where('relation_id', $relation_id)
			                   ->getOne('server_components sc');
	...
	{
		$this->db->insert("server_apps", array("app_id" => $app['app_id'], 'server_id' => $this->server_id, 'damage' => $app['damage']));
	...
	function fetchComponents()
	{
		...
		$this->components = array('rams' => array(), 'hdds' => array());
	...
	{
		if(!$this->skills) $this->fetchSkills();
	...
	function fetchSkills()
	{
		...
		foreach ($this->skills as $skill => &$data)
		{
			...
			while ($data["exp"] >= $exp)
			{
				$lvl++; $exp = $this->uclass->computeSkillExperience($skill, $lvl);
			}
			$data['expNext'] = $exp;
			$data['level'] = $lvl;
		}
	...
	{
		if (!$server_id) $server_id = $this->server_id;
		if (!$server_id) return;
		$this->server_id = $server_id;
		/*******************
		HARDWARE
		*******************/
		...
		{
			$this->disabled = true;
			$this->db->where('server_id', $server_id)->update('server_apps', array('running' => 0));
			//$this->errors[] = "";
		}
		...
		*******************/
		...
		while (count($runningApps) && (($this->cpu_usage > $this->total_cpu) || ($this->ram_usage > $this->total_ram)))
		{
			...
			$this->db->rawQuery('update server_apps set running = 0 where process_id = ? limit 1', array($runningApps[0]['process_id']));
		...
		{ 
			shuffle($apps);
			$deletedApps[] = $apps[0];
			$this->db->rawQuery('delete from server_apps where process_id = ? limit 1', array($apps[0]['process_id']));
		...
		{
			$lostApps = array();
			foreach($deletedApps as $app) $lostApps[] = $app['name'];
	...
	function dealComponentDamage($component, $damage)
	{
		if (!$component['relation_id'])
			$component = $this->db->join('servers s', 's.server_id = sc.server_id', 'left outer')
			                      ->where('relation_id', $component)
			                      ->getOne('server_components sc', 'sc.*, s.user_id');
		else
			$component = array_merge($component, $this->db->where('server_id', $component['server_id'])
				                                          ->getOne('servers', 'user_id'));
		if (!$component['server_id']) return;
		
		$component['damage'] += $damage;
		$component['damage'] = $component['damage'] > 100 ? 100 : $component['damage'];
		$this->db->where('relation_id', $component['relation_id'])
			     ->update('server_components', array('damage' => $component['damage']));
	...
	{
		if (!$app['process_id'])
			$app = $this->db->join('servers s', 's.server_id = sa.server_id', 'left outer')
			            ->where('process_id', $app)
			            ->getOne('server_apps sa', 'sa.server_id, damage, process_id, s.user_id, running');
		else
			$app = array_merge($app, $this->db->where('server_id', $component['server_id'])
				                              ->getOne('servers', 'user_id'));
		
		if (!$app['server_id']) return;
		
		$app['damage'] = $app['damage'] + $damage > 100 ? 100 : $app['damage'] + $damage;

		$this->db->where('process_id', $app['process_id'])
			     ->update('server_apps', array('damage' => $app['damage']));
