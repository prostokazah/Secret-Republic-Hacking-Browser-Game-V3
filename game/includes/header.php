...
require_once('includes/class/RewardsManager.class.php');
require_once('includes/class/cardinal.php');
require_once('includes/class/Item.class.php');

$cardinal = new Cardinal();

require(ABSPATH . 'includes/class/userclass.php');
...
if (!$logged) {
  ...
  if ($_SESSION['showTutorial']) {
    ...
    if ($tutorial['tutorialStep'] = $tutorialSteps[$tutorial['step']]) {
      ...
      $templateVariables['tutorial'] = true;
...
', $tutorialPercent = floor($tutorial["step"] / ($config['tutorialSteps'] / 100)), $tutorial["step"], $config['tutorialSteps']);
		...
		$myModal['id'] = 'tutorial';
		$myModals[] = $myModal;
		$templateVariables['tutorialPercent'] = $tutorialPercent;
