...
class UserClass extends Alpha
{
  ...
  {
    $check = $this->db->where("(user1id = ? and user2id =?) or (user1id = ? and user2id = ?)",
  ...
  function get_friends($id, $limit = '')
  {
    ...
    $friends = $this->db->rawQuery(
    	               'select uf.id,hu.username,hu.zone, hu.lastActive from friendships as uf
                        inner join users  hu
                        on (hu.id=uf.user2id and user1id= ?)
                        or (hu.id=uf.user1id and user2id= ?)
                        order by lastActive desc, username desc ' . $limit , array($id, $id));

    foreach ($friends as &$friend) {
      if ($friend['lastActive'] >= time() - 10 * 60)
        $friend['online'] = true;
  ...
  {
	  $this->db->rawQuery('update users set '.$update.' where id = ? limit 1', array($user));
  ...
  function updatePlayer($updateData, $user_id = false)
  {
    ...
    $this->db->where('id', $user_id)->update('users', $updateData, 1);
  ...
  {
    $job = $this->db->where('user_id', $user_id)
		            ->getOne('user_job uj', 'uj.*'. ($timeBetweenJobs ? ',
    ...
    if (!$job['user_id']) {
      ...
      $this->db->insert('user_job', $insertData);

      $job = $this->db->where('user_id', $user_id)->getOne('user_job');
    } //!$job['user_id']
  ...
  function addEnergy($energy)
  {
    ...
    $this->db->where('id', $this->user['id'])->update('users', array(
  ...
  function addJobExperience($user_id, $experience)
  {
    ...
    if ($job['user_id']) {
      ...
      $this->db->where('user_id', $user_id)->update('user_job', $job);
  ...
  function add_exp($exp)
  {
    ...
    while ($this->user['exp'] >= $this->user['expNext']) {
      ...
      $reward = $this->db->where('level_id', $this->user['level'])->getOne('level_rewards');
      if ($reward['level_id']) {

        if ($reward['achievement_id'])
          $levelReward['achievements'][] = $reward['achievement_id'];
  ...
  function addSkillExperience($uid, $skill, $exp)
  {
    ...
    $this->db->where('uid', $uid)->where('skill', $skill)->update('skills', $skillData);
  ...
  {
	$skills = is_array($skillsToFetch) ? $skillsToFetch : array($skillsToFetch);

	$skillsString = implode(',', $skills);
	$this->db->where('skill in ('. $skillsString .')');


    $skillsData = $this->db->where('uid', $uid)->where('skill', $skill)
		               ->get('skills', is_array($skills) ? count($skills) : 1, 'exp, expNext, level, skill');
	...
	$userSkills = array();
	foreach ($skillsData as $skillData)
		$userSkills[$skillData['skill']] = $skillData;
  ...
  {
	$abilityData = $this->getUserAbilities($user_id, array($ability_id));
  ...
  function getUserAbilities($user_id, $abilitiesArray)
  {
    ...
    $abilities = $this->db->where('user_id', $user_id)
		                  ->where('ability_id in ('.implode(',', $abilitiesArray).')')->get('abilities', null, ' ability_id, level');;
	...
	foreach ($abilitiesArray as $ability)
	{
      ...
      $this->db->insert('abilities', $abilityData);
  ...
  function addAbilityLevel($user_id, $ability_id, $addLevel)
  {
    ...
    $this->db->where('user_id', $user_id)->where('ability_id', $ability_id)->update('abilities', $abilityData);
	...
	if ($sid == -1 && $type == 1)
  {
    ...
    foreach($rid as $r)
      ...
      if ($sid < 0)
      {
        $checkIfConvExists = $this->db->where('receiver_user_id', $r)
                                      ->where('parent_message_id is null')
                                      ->where('sender_user_id', $sid)
                                      ->getOne('conversations', 'message_id');
        if ($checkIfConvExists['message_id'])
        {

          $reply['parent_message_id'] = $checkIfConvExists['message_id'];

          $reply_id = $this->db->insert('conversations', $reply);

          $this->db->rawQuery("update conversations
                         set replies = replies + 1, last_reply_by_user_id = ?, last_reply_date = ?, last_reply_seen = 0
                   where message_id = ? limit 1",
                   array($sid, time(), $checkIfConvExists['message_id']));
          ...
          continue;
			...
			$msg_id = $this->db->insert('conversations', $message);
		}
  ...
  {
	if (!$user_id) return;
    $user = $this->db->where('id', $user_id)->getOne('users', 'rewardsToReceive');
    ...
    $reward_id = $this->db->insert('user_rewards', $insertData);
  ...
  function giveAchievement($uid, $aid, $count = 1) {
    $achievement = $this->db->where('achievement_id', $aid)->getOne('achievements');

    if ($achievement['achievement_id']) {
      $userAchievement = $this->db->where('achievement_id', $achievement['achievement_id'])->where('user_id', $uid)->getOne('user_achievements');
      ...
      if ($userAchievement['user_achievement_id']) {
        ...
        $this->db->where('achievement_id', $achievement['achievement_id'])->where('user_id', $uid)->update('user_achievements', $update);
        ...
        $this->db->insert('user_achievements', $userAchievement);
  ...
  function getPremiumData($uid = false)
  {
  	...
  	$premium = $this->db->where('user_id', $uid)->getOne('user_premium');
  	if (!$premium['id'])
    	$premium['id'] = $this->db->insert('user_premium', array('user_id' => $uid));
	...
	function getUserStorage($user_id = false, $in = array(), $onlyDamaged = false)
	{
		if (!$user_id) $user_id = $this->user['id'];
		$storage =
		 $this->db->rawQuery('select c.*, s.* from storage s
	                          left outer join components c on c.component_id = s.component_id
							  where s.user_id = ? '
							 .(count($in) ? ' and s.storage_id in ('.implode(',', $in).')' : '')
							 .($onlyDamaged ? ' and s.damage != 0 and s.locked is null' : '')
							  . ' order by c.type asc',
							 array($user_id));
	...
	{
		$component = $this->db->join('components c', 'c.component_id = s.component_id' , 'left outer')
	...
	{
		$referral = $this->db->where('slave_user_id', $this->user['id'])
			                 ->getOne('user_referrals', 'referral_id, master_user_id');
		if ($referral['referral_id'])
			if ($this->user['level'] == 10)
			{
				$this->db->where('referral_id', $referral['referral_id'])->update('user_referrals', array('status' => 2), 1);
				$reward = array('alphaCoins' => 5, 'referral_id' => $referral['referral_id']);
				...
				$referralsBatch = $this->db->where('master_user_id', $referral['master_user_id'])
					                       ->where('status', 2)
					                       ->getOne('user_referrals', 'count(*) nrr');
				if ($referralsBatch['nrr'] == 5)
				{
					$reward = array('alphaCoins' => 5, 'referral_id' => $referral['referral_id']);
					$this->db->where('master_user_id', $referral['master_user_id'])
						     ->where('status', 2)
						     ->update('user_referrals', array('status' => 3));
	...
	function alphaCoinsPurchase($user_id, $alphaCoins)
	{
		...
		*/
    $u = $this->db->where('uid', $user_id)->getOne('user_credentials', "email");

		$check = $this->db->where('user_id', $user_id)->getOne('alpha_coin_transactions', 'transaction_id');
		if (!$check['transaction_id']) $alphaCoins += floor(($alphaCoins / 100) * 25);

    $alphaCoins = intval($alphaCoins);
		$this->db->rawQuery('update users set alphaCoins = alphaCoins + ? where id = ? limit 1',
						     array($alphaCoins, $user_id));

    $replace = array("ALPHA_COINS");
    $with    = array($alphaCoins);
    $email   = $this->getEmailTemplate("alpha_coins", $replace, $with);

    $email['recipients'] = array($u['email']);
    ...
    $this->sendEmail($email);
		...
		$user = $this->db->where('id', $user_id)->getOne('users', 'level');
		if ($user['level'] < 4) return;

		$referral = $this->db->where('slave_user_id', $user_id)->getOne('user_referrals', 'master_user_id, referral_id');
		if (!$referral['referral_id']) return;

		$reward = array('alphaCoins' => $rewardTier1, 'referral_id' => $referral['referral_id']);
		$this->addReward($referral['master_user_id'], $reward, 'A referral has purchased A-Coins');

		if (!$rewardTier2) return;
		$referralTier2 = $this->db->where('slave_user_id', $referral['master_user_id'])
			                      ->getOne('user_referrals', 'master_user_id, referral_id');

		if (!$referralTier2['referral_id']) return;

		$reward = array('alphaCoins' => $rewardTier2, 'referral_id' => $referralTier2['referral_id']);
    ...
    $alphaCoins = $this->db->where('id', $user_id)->getOne('users', 'alphaCoins');
    ...
    if ($alphaCoins['alphaCoins'] < $amount) return false;
		...
		if(!$this->db->insert('alpha_coins_logs', $logInsert)) return false;

    $this->updatePlayer(array('alphaCoins' => $alphaCoins['alphaCoins'] - $amount), $user_id);
	...
	{
	  if (!$reward) $reward = $this->db->where('reward_id', $reward_id)->getOne('user_rewards');
	  if (!$reward['reward_id']) return false;

	  $user = $this->db->where('id', $reward['user_id'])->getOne('users', 'id, money, alphaCoins,skillPoints, rewardsToReceive, dataPoints');
	  if (!$user['id']) return false;

	  if ($reward['components'])
	  {
	  	$components = unserialize($reward['components']);
	    if ($this->uclass->getAvailableStorageSlots() < count($components))
  			add_alert('Not enough slots available to claim the hardware.');
	  }
      ...
      if ($server->server['total_hdd'] - $server->server['hdd_usage'] < $neededHDD)
        add_alert("Not enough HDD on main server to store the apps");
	  }
    ...
    if (!there_are_errors())
    {
        ...
        {
          $newItem = array('component_id' => $component['component_id'],
                   'damage' => $component['damage'],
                   'user_id' => $this->user['id']);

          $this->db->insert('storage', $newItem);
        }
        ...
        if ($reward['exp'])
          $this->add_exp($reward['exp']);

        if ($reward['jobExp'])
          $this->addJobExperience($reward['user_id'], $reward['jobExp']);

        if ($reward['energy'])
          $this->addEnergy($reward['user_id'], $reward['energy']);
  	  ...
  	  $this->updatePlayer(array('money' => $user['money'],
        ...
        if (is_array($reward['skills']))
          foreach($reward['skills'] as $skill => $amount)
            $this->addSkillExperience($reward['user_id'], $skill, $amount);

        if (is_array($reward['achievements']))
          foreach($reward['achievements'] as $achievement)
            $this->giveAchievement($reward['user_id'], $achievement);

      $reward['received'] = time();
        $this->db->where('reward_id', $reward['reward_id'])->update('user_rewards', array('received' => $reward['received']));
	...
	function computeUserSkillsCommandsInfluence($user_id = false, $skills = false, $allServers = false)
	  {
		...
		{
			$user = $this->db->where('id', $user_id)->getOne('users', 'id, server');
			$main = $user['server'];
		}
		...
		if (!$skills) {
		  ...
		  $skills        = $this->uclass->getUserSkills($user_id, $skillsToFetch);
		} //!$skills
		...
		if ($main)
		{
			...
			if ($allServers)
			{
				$servers = $this->db->where('user_id', $user_id)->get('servers');
	...
	function recomputeDataPointsStats($user_id)
	{
		...
		$this->updatePlayer(array('dataPointsPerHour' => $dataPointsPerHour), $user_id);
	...
	function getAvailableStorageSlots()
	{
		...
		$used = $this->db->where('user_id', $user_id)->getOne('storage', 'count(*) nrs');
  ...
  function substractResources($resources, $user_id)
  {
    ...
    $this->db->rawQuery('update users set energy = energy - ?,
                                          money = money - ?,
                                          dataPoints = dataPoints - ?
                          where id = ? limit 1',
                          array($resources['energy'], $resources['money'], $resources['dataPoints'],
                                $user_id));
  }
	...
	function sendAchievements($achievements = array(), $user_id = false, $unique = false, $title = "Achievement")
	{
		...
		if ($unique)
	    {
		  ...
		  {
		  	$check = $this->db->where('user_id', $user_id)->where('achievement_id', $achievement)->getOne('user_achievements', 'user_achievement_id');
		  	if ($check['user_achievement_id']) unset($achievements[$key]);
		  }
		}


		$reward = array("achievements" => array(3));
		$this->uclass->addReward($user_id, $reward, $title);
	...
	function getAvailableServersForAttack($user_id = false)
	{
		...
		$servers = $this->db->where('user_id', $user_id)->get("servers");
	  $usedServers = $this->db->where('sender_user_id', $user_id)->where('servers is not null')->get('attacks_inprogress');
	  $usedServersIds = array();
	  foreach ($usedServers as $inprogressAttack)
		  $usedServersIds = array_merge($usedServersIds, explode(",", $inprogressAttack['servers']));

	  foreach($servers as &$server)
		  if (in_array($server['server_id'], $usedServersIds))
			$server['cant_use'] = true;
